...
template<typename T, dim_t D> static void sortPoints(const SpIt<T,D>& beg, const SpIt<T,D>& end, dim_t coord) {
...
///////////////////////////////////////////////////////////////////////////////
template<typename T, dim_t L> class InnerNode : public Node<T, L> {
...
public:
	...
	void print(std::ostream& os) const override { m_left->print(os); os << ",{"; this->assoc()->print(os); os << "},"; m_right->print(os); }
};

///////////////////////////////////////////////////////////////////////////////
template<typename T, dim_t L, dim_t D> class LeafNode : public Node<T, L> {
	using AssocUP = std::unique_ptr<Node<T, L-1>>;
...
///////////////////////////////////////////////////////////////////////////////
template<typename T, dim_t L, dim_t D=L> class RangeTree {
	using AssocUP = std::unique_ptr<Node<T, L-1>>;
	using NodeUP = std::unique_ptr<Node<T, L>>;
	using NodePtr = const Node<T, L>*;
...
public:
	...
	static NodeUP buildTree(const SpIt<T, D>& beg, const SpIt<T, D>& end) {
		if (end - beg == 1) {
			...
			const T key = (**(m - 1))[D - L];	// must be called before buildAssocTree, because it changes order of points
			auto left = buildTree(beg, m);		// must be called before buildAssocTree, because it changes order of points
			auto right = buildTree(m, end);		// must be called before buildAssocTree, because it changes order of points
...
///////////////////////////////////////////////////////////////////////////////
template<typename T> class InnerNode<T, 1> : public Node<T, 1> {
...
public:
	...
	NodePtr right() const { return m_right.get(); }
	const T& key() const override { return m_key; }
	void print(std::ostream& os) const override { m_left->print(os); os << ','; m_right->print(os); }
};

///////////////////////////////////////////////////////////////////////////////
template<typename T, dim_t D> class LeafNode<T,1,D> : public Node<T, 1> {
	std::shared_ptr<Point<T, D>> m_point;
...
///////////////////////////////////////////////////////////////////////////////
template<typename T, dim_t D> class RangeTree<T, 1, D> {
	using NodeUP = std::unique_ptr<Node<T, 1>>;
	using NodePtr = const Node<T, 1>*;
