...
#include "../RangeQuery/RangeQuery.hpp"
#include <algorithm>
#include <vector>
#include <sstream>
#include <random>
...
/// <summary>
/// Writes a point vector to a wide-string.
/// </summary>
/// <typeparam name="P">point data type</typeparam>
/// <param name="v">point vector</param>
/// <returns>wide string</returns>
template<typename P>
std::wstring toString(const vector<P>& v) {
	wstringstream wss;
	wss << '[';
	if (!v.empty()) wss << v[0];
	for (size_t i = 1; i < v.size(); i++) wss << ',' << v[i];
	wss << ']' << endl;
...
namespace UnitTest
{
	...
	public:
		...
		TEST_METHOD(Simple1D)
		{
			...
			sort(v.begin(), v.end());
			Assert::AreEqual(vector<Point1>({ 2,4,5,8,9 }), v);
		}
		TEST_METHOD(Duplicates1D)
		{
			...
			sort(v.begin(), v.end());
			Assert::AreEqual(vector<Point1>({ 2,2,4,4,5,5,8,8,9,9,9 }), v);
		}
		TEST_METHOD(Random1D)
		{
		...
		TEST_METHOD(Simple2D)
		{
			vector<Point2> v({ {4,6}, {1,5}, {2,7}, {3,8}, {1,1}, {2,5}, {6,1}, {4,4} });
			...
			sort(v.begin(), v.end());
			Assert::AreEqual(vector<Point2>({ {1,1}, {1,5}, {2,5}, {2,7}, {3,8}, {4,4}, {4,6}, {6,1} }), v);
		}
		TEST_METHOD(Duplicates2D)
		{
			vector<Point2> v({ {4,6}, {1,5}, {2,7}, {3,8}, {1,1}, {2,5}, {6,1}, {4,4}, {1,5}, {2,7}, {3,8}, 
				{1,1}, {2,5}, {6,1}, {4,4}, {4,4}, {1,5}, {2,7}, {3,8}, {1,1}, {2,5} });
			RangeQuery<Point2> rq(v);
			...
			sort(v.begin(), v.end());
			Assert::AreEqual(vector<Point2>({ {1,1}, {1,1}, {1,1}, {1,5}, {1,5}, {1,5}, {2,5}, {2,5}, {2,5}, 
				{2,7}, {2,7}, {2,7}, {3,8}, {3,8}, {3,8}, {4,4}, {4,4}, {4,4}, {4,6}, {6,1}, {6,1} }), v);
		}
		TEST_METHOD(Random2D)					   
		{										   
		...
		TEST_METHOD(Simple3D)
		{
			vector<Point3> v({ {4,6,4.5}, {1,5,4}, {2.5,7,6}, {3,8,3}, {1,1.5,5}, {2.5,5.5,1}, {6,1,2}, {4,4,7} });
			...
			sort(v.begin(), v.end());
			Assert::AreEqual(vector<Point3>({ {1,1.5,5}, {1,5,4}, {2.5,5.5,1}, {2.5,7,6}, {3,8,3}, {4,4,7}, {4,6,4.5}, {6,1,2} }), v);
		}
		TEST_METHOD(Duplicates3D)
		{
			vector<Point3> v({ {4,6,4.5}, {1,5,4}, {2.5,7,6}, {3,8,3}, {1,1.5,5}, {2.5,5.5,1}, {6,1,2}, {4,4,7}, {4,6,4.5}, 
				{1,5,4}, {2.5,7,6}, {3,8,3}, {1,1.5,5},  {1,1.5,5}, {2.5,5.5,1}, {6,1,2}, {4,4,7}, {4,6,4.5}, {1,5,4} });
			RangeQuery<Point3> rq(v);
...
template<> inline std::wstring Microsoft::VisualStudio::CppUnitTestFramework::ToString(const vector<Point1>& v) {
	return toString(v);
}
template<> inline std::wstring Microsoft::VisualStudio::CppUnitTestFramework::ToString(const vector<Point2>& v) {
	return toString(v);
}
template<> inline std::wstring Microsoft::VisualStudio::CppUnitTestFramework::ToString(const vector<Point3>& v) {
	return toString(v);
}
